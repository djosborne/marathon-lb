...
'''
    ...
    HAPROXY_BACKEND_PORTLESS_SERVER_OPTIONS = '''\
  server {serverName} {host_ipv4}{cookieOptions}{healthCheckOptions}\
{otherOptions}
'''
...
'''
    ...
    def haproxy_backend_portless_server_options(self, app):
        if 'HAPROXY_{0}_BACKEND_PORTLESS_SERVER_OPTIONS' in app.labels:
            return app.labels['HAPROXY_{0}_BACKEND_PORTLESS_SERVER_OPTIONS']
        return self.HAPROXY_BACKEND_PORTLESS_SERVER_OPTIONS
    ...
    for app in sorted(apps, key=attrgetter('appId', 'servicePort')):
        ...
        for backendServer in sorted(app.backends, key=key_func):
            ...
            if app.ip_per_container:
                ...
                if not ipv4:
                    print("App requested IP-per-container, but has not yet been assigned an IP. skipping.")
                    continue
                port = backendServer.container_port
            else:
                ipv4 = resolve_ip(backendServer.host)
                port = backendServer.port
            if ipv4 is not None and port is not None:
            ...
            elif ipv4 is not None and port is None:
                backend_portless_server_options = templater \
                    .haproxy_backend_portless_server_options(app)
                backends += backend_portless_server_options.format(
                    host=backendServer.host,
                    host_ipv4=ipv4,
                    serverName=serverName,
                    cookieOptions=' check cookie ' +
